...
class convmodel():
    ...
    def _denseblock(self, input, kernel1, kernel2, layer_name, is_training):
        ...
        with tf.name_scope(layer_name):
            ...
            # c = self.SE_block(c,c._shape_as_list()[-1],32,layer_name+'SE')
    ...
    def convnet(self):
        ...
        # if expand_dims: conv = tf.expand_dims(conv, axis=0)

        onehot_labels = tf.one_hot(indices=id, depth=3, on_value=1.0, off_value=0.0)
        ...
        return image, id, loss, acc, is_training, tf.nn.softmax(conv, dim=-1) #, expand_dims
    ...
    def convnet_test(self):
        ...
        return image, is_training, tf.nn.softmax(conv, dim=-1),output,output2 #, expand_dims
